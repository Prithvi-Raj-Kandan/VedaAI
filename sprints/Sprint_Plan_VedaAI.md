...
**Progress Update:**
- The backend now stores assignments under a `userId` owner and filters history/regenerate/version reads by the current user.
- The v2 assignments page now loads the logged-in user's assignments from MongoDB instead of mock data.
- History cards open the owned assignment directly so users can reopen prior papers.
...
**Progress Update:**
- Added a MongoDB `User` model and linked each assignment to a `userId` owner.
- Scoped assignment fetch/list/regenerate/version routes to the current resolved user.
- Added a `/api/user/me` endpoint for profile/history bootstrap work.
- The v2 settings page now works as a real profile/auth screen with sign up, sign in, sign out, and current user details.
- Cleared the local MongoDB and Redis dev data so the new user-scoped schema starts clean.
...
**Progress Update:**
- The worker now emits and persists progress milestones for PDF processed, questions drafted, sections finalized, and paper saved.
- The v2 loading screen renders the staged progress instead of a single spinner.
- The assignment route still opens the intermediate waiting page while generation completes.
